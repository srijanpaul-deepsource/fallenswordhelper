...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-a35a8c43.js')); };
const injectGuild = () => { runDefault(import('./guild-8f38a45a.js')); };
const injectProfile = () => { runDefault(import('./profile-33e5ae74.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-ea0037cb.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-5bcc185a.js')); };
const inventing = () => { runDefault(import('./inventing-5b74d4d2.js')); };
const news$1 = () => { runDefault(import('./news-ae4e4800.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-7de6d134.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-6bca605e.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-57319cd5.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-d001f6c2.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-ba0e250b.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-b0c1aae2.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-1fed50be.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-d0790368.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-a3740180.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-f292009b.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-80057096.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-80057096.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-cfc2893b.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-cfc2893b.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-abe99c9c.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-54539ab6.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-1e6cc612.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-a53f02b7.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-489f1d8f.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-0217fc9a.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-9bc28143.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-187417dc.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-663a6ad8.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-4887f802.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-ab22fde1.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-8dc44fae.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-61b25b53.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-a4748f6c.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-db22ae5f.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-2cf0502f.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-2d028191.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-40153d11.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-7067d382.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-2d6c84fa.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-01527d91.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-60e92db8.js').then(function (n) { return n.b; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-d6951937.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-02c7e3ce.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-a2756a84.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-71fdcb99.js'));
};
const storeMoves = () => { runDefault(import('./store-3d80ca4d.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-98ec2938.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-acf6b9df.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-5b123cc0.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-bf501d69.js')); };
const injectComposing = () => { runDefault(import('./composing-67fd5371.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-0d7b3727.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-489d53b2.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-bfb6c49c.js'));
};
...
const guildHall = () => { runDefault(import('./hall-7d6cabc7.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-0464b594.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-bfeef62f.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-890ac72e.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-fe3eea16.js')); };
const guildLog = () => { runDefault(import('./guildLog-1d697828.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-cce42ce6.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-af51a8f8.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-2ca0aba5.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-31342a17.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-c8eace18.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-25448691.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-cf5d462a.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-5e74dce2.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-013f64ec.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-f92638ee.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-c68ac917.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-6a24d039.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-fcfddb9e.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-e6ccc98c.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-3b5613ce.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-3e32a091.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-cf7ddec3.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-7d33dd98.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-fdf40f9e.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-8f3667e2.js')); };
const injectTopRated = () => { runDefault(import('./toprated-3cedb5ac.js')); };
...
const injectTrade = () => { runDefault(import('./trade-5a8dc5f4.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-2f5a8343.js')); };
const injectBank = () => { runDefault(import('./injectBank-19986667.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-e7695996.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-7fb71328.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-e09f6768.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-a64a201a.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-be2e7eeb.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-fdb484a3.js'));
};
const ladder = () => { runDefault(import('./ladder-55634086.js')); };
const marketplace = () => { runDefault(import('./marketplace-cf26099b.js')); };
const points = () => { runDefault(import('./points-eef66630.js')); };
const superelite = () => { runDefault(import('./superelite-631ca5fd.js')); };
const privateMsg = () => { runDefault(import('./privateMsg-0e8e2fd5.js')); };
const playerLog = () => { runDefault(import('./playerLog-7949f029.js')); };
...
//# sourceMappingURL=calfSystem-6b7d7ae6.js.map
