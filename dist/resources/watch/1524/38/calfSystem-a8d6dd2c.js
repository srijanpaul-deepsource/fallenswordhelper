...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-80db9b73.js')); };
const injectGuild = () => { runDefault(import('./guild-77bdd2a4.js')); };
const injectProfile = () => { runDefault(import('./profile-2d30e3e2.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-eef5487e.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-841e54b7.js')); };
const inventing = () => { runDefault(import('./inventing-48b1f2d1.js')); };
const news$1 = () => { runDefault(import('./news-6d102328.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-cccb04b0.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-0ce52d88.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-ada5602e.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-60eb57e2.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-002b8d90.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-c6174060.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-ea1bd6f6.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-d40f1880.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-f947113f.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-0a069bae.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-af083b5c.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-af083b5c.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-053b13cf.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-053b13cf.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-68b8c5d0.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-86d18278.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-d44cabef.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-1f3ff602.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-2ac3cb3f.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-2357da54.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-4c1f9c96.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-c139de4f.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-e3fed955.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-2efd45e8.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-7eafa386.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-fb45008f.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-13bdd372.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-648a2cbe.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-99678ab4.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-ed8622e3.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-eca69ffc.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-08642169.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-ccc10595.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-61ebdeca.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-a39ef494.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-569f1af8.js').then(function (n) { return n.b; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-b138178a.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-c7cc4f34.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-98d5175b.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-b801101b.js'));
};
const storeMoves = () => { runDefault(import('./store-b2bdffe8.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-639eddda.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-54b0b4ef.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-06c45902.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-bdbc2b5f.js')); };
const injectComposing = () => { runDefault(import('./composing-4d796d7b.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-a047d86f.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-1c3c3fbe.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-183eb3fe.js'));
};
...
const guildHall = () => { runDefault(import('./hall-0aa1745b.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-307707aa.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-6363b969.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-79cc4195.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-952d600f.js')); };
const guildLog = () => { runDefault(import('./guildLog-dc76f7e2.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-63783497.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-8c59a344.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-9341e221.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-eb0d1a78.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-5afc1030.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-468914b6.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-3ef80159.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-1e1c5d30.js')); };
const outbox = () => { runDefault(import('./outbox-ee37bf4c.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-791d899f.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-99a4720c.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-4babc3ec.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-de831ebb.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-a808e9f1.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-a5cd03c2.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-2181c107.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-adac184c.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-a4f5f84d.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-8ff98e9d.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-c1e6257b.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-e58dbaf2.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-0c35d9ff.js')); };
const injectTopRated = () => { runDefault(import('./toprated-68394178.js')); };
...
const injectTrade = () => { runDefault(import('./trade-21b4bb58.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-c989e2f1.js')); };
const injectBank = () => { runDefault(import('./injectBank-508e8f22.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-0052fdff.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-9757852b.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-cfeb87cf.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-b819fd52.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-d4bc84e1.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-6e8ebbe4.js'));
};
const ladder = () => { runDefault(import('./ladder-707df73a.js')); };
const marketplace = () => { runDefault(import('./marketplace-266bd944.js')); };
const points = () => { runDefault(import('./points-65d7f1d6.js')); };
const superelite = () => { runDefault(import('./superelite-601114e5.js')); };
...
//# sourceMappingURL=calfSystem-a8d6dd2c.js.map
