...
function retrieve(value) {
  const test = reviver$1.find((el) => value.startsWith(el[0]));
...
async function runDefault(prm) {
  const mod = await prm;
  mod.default();
}

const injectBioWidgets = () => { runDefault(import('./bioWidgets-64cee5f0.js')); };
const injectGuild = () => { runDefault(import('./guild-888cd5b7.js')); };
const injectProfile = () => { runDefault(import('./profile-54f1855e.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-02e0cee1.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-90a7f397.js')); };
const inventing = () => { runDefault(import('./inventing-cec2251a.js')); };
const news$1 = () => { runDefault(import('./news-b9f7f827.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-8d85ef5b.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-d8baf584.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-9ea26bf6.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-904abbd2.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-0d543d4f.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-e5ceab45.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-c82fa686.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-7759f60d.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-4abd4dcf.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-237190ec.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-37298c97.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-37298c97.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-b4de2374.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-b4de2374.js').then((m) => m.injectFindOther(i)); };
...
var css$1 = "#content { /* csslint allow: ids */\n  max-width:600px\n}\n";
var modules_419041ea = {};
...
function asyncPThree(fnList) {
  fnList.forEach((fn) => add$1(3, fn));
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-aaf551d4.js'));
  }
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-6ea8febe.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-97f6e020.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-8ad0bf84.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-af41316a.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-b2c225e1.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-9682a148.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-334f8e67.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-3e717425.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-65b92038.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-a81b8264.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-d26611b2.js'));
  }
}
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-89e72487.js')
...
function classPair(element, pair) { return hasClass(pair[0], element); }
...
function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-bcf24803.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-cea50fe0.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-9d9047e5.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-a2ef623e.js').then((m) => m.seLog());
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-3ed1743d.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-2b97e3c7.js'));
  }
}

function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-a15ac454.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-729469a3.js').then((m) => m.default(title));
}
...
function profile$2(data) {
  return callApp(extend({ cmd: 'profile' }, data));
}

async function viewcombatset() {
  const json = await profile$2({ subcmd: 'viewcombatset' });
...
function daViewCombatSet() {
  return $dataAccess(viewcombatset, viewCombatSets);
}
...
async function combatSetKey(itemIndex) {
...
function betaWarning() {
  ...
  if (calf.fshVer.length === 5 && lastBetaWarning < nowSecs - 24 * 60 * 60) {
    dialogMsg$1('<b>Beta version is deprecated.</b><br>'
...
const injectArena = () => { runDefault(import('./arena-cce6629a.js').then(function (n) { return n.b; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-57e30e99.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-f6f1c98c.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-e34fb686.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-19c3dfc5.js'));
};
const storeMoves = () => { runDefault(import('./store-758effd7.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-869ca0ed.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-f699f3a6.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-a1dd161c.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-33e76400.js')); };
const injectComposing = () => { runDefault(import('./composing-3085f98a.js')); };
...
const imgFix = () => { runDefault(import('./imgFix-96023226.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-3163e5f0.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-fd16f3c5.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-d10d3ee0.js'));
};
...
const guildHall = () => { runDefault(import('./hall-5cbeb7d8.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-f004b681.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-40759cbb.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-779fbe2d.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-d9c0abcb.js')); };
const guildLog = () => { runDefault(import('./guildLog-ce63e517.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-9fbbeb53.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-6ded744d.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-12b576d8.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-8d87bb57.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-8d2bcc7e.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-9331c7e0.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-7add9c28.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-66700ee0.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-25ed6f6d.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-93ff6763.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-e6638cb9.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-c63047ff.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-a0cd48fe.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-0c463a64.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-15abf765.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-af46ffa8.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-7b2232e5.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-5d6a1084.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-aa881978.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-f289d71c.js')); };
const injectTopRated = () => { runDefault(import('./toprated-a520fe98.js')); };
...
const injectTrade = () => { runDefault(import('./trade-765cf1cb.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-47a2f196.js')); };
const injectBank = () => { runDefault(import('./injectBank-f30ac60b.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-efac8649.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-c3f82292.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-292d473d.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-5b5075e9.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-f6e6335f.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-0ec25245.js'));
};
const ladder = () => { runDefault(import('./ladder-05335856.js')); };
const marketplace = () => { runDefault(import('./marketplace-2f70c060.js')); };
const points = () => { runDefault(import('./points-32251da3.js')); };
const superelite = () => { runDefault(import('./superelite-0af160f5.js')); };
const privateMsg = () => { runDefault(import('./privateMsg-40ca6468.js')); };
const playerLog = () => { runDefault(import('./playerLog-c20522ce.js')); };
const injectBuffmarket = () => { runDefault(import('./injectBuffmarket-5f68a6ff.js')); };
...
export { $dataAccess as $, setText as A, setInnerHtml as B, getText as C, searchPlayerUrl as D, querySelector as E, querySelectorArray as F, playerLinkSelector as G, getTextTrim as H, getValue as I, defCharacterVirtualLevel as J, defStatLevel as K, quickbuffUrl as L, getElementsByClassName as M, navigateTo as N, indexPhp as O, isArray as P, once as Q, clickThis as R, getUrlParameter as S, sendEvent as T, insertHtml as U, nowSecs as V, scouttowerUrl as W, setValue as X, dialogMsg$1 as Y, jQueryDialog as Z, injectNotepadShowLogs as _, add$1 as a, hideQTip as a$, injectMonsterLog as a0, getCalfPrefs as a1, escapeHtml as a2, theLinks as a3, playerIdUrl as a4, notepadBlankUrl as a5, fallback as a6, isBoolean as a7, isUndefined as a8, now as a9, loadCss as aA, infoBox as aB, cdn as aC, arenaUrl as aD, oldActionSpinner as aE, defTd as aF, runDefault as aG, composingUrl as aH, defSubcmd as aI, defNeedToCompose as aJ, contains as aK, defLastComposeCheck as aL, playerId as aM, showPlayerUrl as aN, retryAjax as aO, isFunction as aP, containsText as aQ, guideUrl as aR, defFetchPlayerBuffs as aS, defPlayerBuffs as aT, defPlayerUpdate as aU, defTeleport as aV, defPvE as aW, defAfterUpdateActionlist as aX, guildViewUrl as aY, defFetchWorldRealmDynamic as aZ, defFetchWorldRealmActions as a_, defLastLadderReset as aa, server as ab, cmdUrl as ac, pointsUrl as ad, insertHtmlAfterBegin as ae, querySelectorAll as af, months as ag, sendException as ah, parseError as ai, arrayFrom as aj, toLowerCase as ak, classPair as al, tradeUrl as am, secureUrl as an, isObject as ao, getCustomUrlParameter as ap, attackplayerUrl as aq, doAddIgnore as ar, fromEntries as as, combatSelector as at, noteSelector as au, defTr as av, defaults as aw, time as ax, timeEnd as ay, loadScript as az, createDiv as b, newGuildLogLoc as b$, worldUrl as b0, defRealmUpdate as b1, defPlayerLevel as b2, keys as b3, defenderMultiplier as b4, defRelicView as b5, doSendGold as b6, defPlayerGold as b7, initSendGoldOnWorld as b8, sendGoldonWorld as b9, guildLogUrl as bA, GMSTORAGE_PATH as bB, lastActivityRE as bC, playerIDRE as bD, defCmd as bE, alpha as bF, guildRE as bG, defEnableGuildActivityTracker as bH, draggable as bI, recallUserUrl as bJ, defStatVl as bK, asyncPThree as bL, updateArchiveUrl as bM, archiveUrl as bN, auctionSearchUrl as bO, getValueJSON as bP, setValueJSON as bQ, profileUrl as bR, pCR as bS, pCL as bT, defJoinallgroupsundersize as bU, classHandler as bV, injectGuild as bW, bountyUrl as bX, parseTemplePage as bY, displayDisconnectedFromGodsMessage as bZ, injectFsBoxContent as b_, defStatAttack as ba, defStatDefense as bb, defStatArmor as bc, defStatDamage as bd, defStatHp as be, defRefreshActionList as bf, defStairway as bg, defShopPrompt as bh, defViewCreature as bi, jsonParse as bj, defFetchPlayerBackpackCount as bk, defControlsKeydown as bl, handleEvent as bm, places as bn, mercRE as bo, ahSearchUrl as bp, fshBuffLog as bq, screenview as br, news$1 as bs, injectQuestBookFull as bt, inventing as bu, rarity as bv, itemType as bw, profile$2 as bx, composingFragmentType as by, isString as bz, calf as c, newGuildLogUrl as c0, injectAuctionSearch as c1, injectOnlinePlayers as c2, injectFindOther as c3, injectFindBuffs as c4, injectRecipeManager as c5, injectBuffLog as c6, injectQuickLinkManager as c7, joinallUrl as c8, joinUnderUrl as c9, defStatbarLevel as ca, blacksmithUrl as cb, dropItemsUrl as cc, joinGroups as cd, auctionhouseUrl as ce, insertQuickExtract as cf, insertQuickWear as cg, dispatch as ch, defTable as d, entries as e, insertHtmlBeforeEnd as f, getElementsByTagName as g, hasClass as h, insertElement as i, jQueryPresent as j, on as k, itemRE as l, getArrayByTagName as m, cElement as n, onclick as o, pCC as p, extend as q, indexAjax as r, partial as s, createDocument as t, indexAjaxData as u, guildSubcmdUrl as v, infoBoxFrom as w, callApp as x, jQueryNotPresent as y, getElementById as z };
//# sourceMappingURL=calfSystem-8168cf6d.js.map
