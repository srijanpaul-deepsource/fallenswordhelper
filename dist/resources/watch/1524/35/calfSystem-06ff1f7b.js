...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-640f59e4.js')); };
const injectGuild = () => { runDefault(import('./guild-a6bd7f40.js')); };
const injectProfile = () => { runDefault(import('./profile-0e974d06.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-18f4555f.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-30801972.js')); };
const inventing = () => { runDefault(import('./inventing-d9dd12ea.js')); };
const news$1 = () => { runDefault(import('./news-31386cd3.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-62b9046c.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-9f1dab22.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-de073ec4.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-2341f451.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-6063f112.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-a46d81d6.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-9b359d52.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-613652cb.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-b584aa07.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-1682b7c8.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-1728df73.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-1728df73.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-7fb4c468.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-7fb4c468.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-f8306b60.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-ac4a1418.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-c923d0a2.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-d44f8f2f.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-ed679e7c.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-2a2c462b.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-e9acba7b.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-65ea5ef9.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-e85115a3.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-c51f4f8e.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-fb846c4c.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-5805e72d.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-0e248917.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-a49b01b9.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-95dbd047.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-1d8c7090.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-98853081.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-4d3b4fc5.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-c8a403f0.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-52efcf4f.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-3f91f45d.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-a0052dd7.js').then(function (n) { return n.b; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-0a58dae1.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-d302d5da.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-92fdcd18.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-19b23c86.js'));
};
const storeMoves = () => { runDefault(import('./store-aa55b1d1.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-a8d77917.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-55c7cb29.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-a582e1ba.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-38a47664.js')); };
const injectComposing = () => { runDefault(import('./composing-9ec0f7c4.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-087ae8ac.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-61a33a21.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-940b9ea3.js'));
};
...
const guildHall = () => { runDefault(import('./hall-3f4120a4.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-a7ac594f.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-f0444325.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-ffde104a.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-e2044281.js')); };
const guildLog = () => { runDefault(import('./guildLog-94bc1481.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-0e6a2a13.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-eb8350e3.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-47bbc2d0.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-a66b6dd8.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-af1af5ec.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-58ffe4bb.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-bc60a51f.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-4fe95b82.js')); };
const outbox = () => { runDefault(import('./outbox-b6ca40af.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-807d3a31.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-f2a1bca5.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-19d6b499.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-77f0b359.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-d66f40e4.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-88974277.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-c5632664.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-cf90f8c3.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-bc2c554d.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-8b67c1a7.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-a4f47322.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-b0b4b1ea.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-0007624e.js')); };
const injectTopRated = () => { runDefault(import('./toprated-1311381f.js')); };
...
const injectTrade = () => { runDefault(import('./trade-a26685e8.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-6e87dde5.js')); };
const injectBank = () => { runDefault(import('./injectBank-154d7031.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-fd0c4717.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-303c78bb.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-36d873eb.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-4c3aa39d.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-63d66486.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-6bedf191.js'));
};
const ladder = () => { runDefault(import('./ladder-14093ea0.js')); };
const marketplace = () => { runDefault(import('./marketplace-7a9a9027.js')); };
const points = () => { runDefault(import('./points-4628b98a.js')); };
const superelite = () => { runDefault(import('./superelite-78f020d7.js')); };
...
//# sourceMappingURL=calfSystem-06ff1f7b.js.map
