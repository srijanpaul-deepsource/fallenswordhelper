...
function timeEnd(name) {
  if (timers[name]) {
    log$1(name, `${Math.round(performance.now() * 1000
      - timers[name]) / 1000}ms`);
    delete timers[name];
  }
}
...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-f1014eeb.js')); };
const injectGuild = () => { runDefault(import('./guild-4ac42ad9.js')); };
const injectProfile = () => { runDefault(import('./profile-a1504ca2.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-d2263acb.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-1b95df92.js')); };
const inventing = () => { runDefault(import('./inventing-8730da49.js')); };
const news$1 = () => { runDefault(import('./news-b80d6341.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-d8038626.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-86435679.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-1db39356.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-2d743348.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-35cb7c3c.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-c3ed2e4a.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-b912ab43.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-636b4c9e.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-533875c1.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-5bdd2f63.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-fabcd61d.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-fabcd61d.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-2c432aaf.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-2c432aaf.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-88feb73a.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-3d1c9ebc.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-3e898c1f.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-2005e116.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-7587e5fb.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-afb0b80a.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-b6d4d4b5.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-320359d9.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-f7fc5a6d.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-a7bee256.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-9d6086bc.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-1137c1cf.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-8ac98be1.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-50263452.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-b7a37635.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-36420b84.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-97e36846.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-21285663.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-19aa8ee6.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-063307cc.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-37ca2848.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-028c7aa9.js').then(function (n) { return n.b; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-9c64b7eb.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-e8873a56.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-4e1a3880.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-98fb1fa9.js'));
};
const storeMoves = () => { runDefault(import('./store-e8f8664f.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-899de7b3.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-430b75e9.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-6f0266e7.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-3ae39d6e.js')); };
const injectComposing = () => { runDefault(import('./composing-e162831d.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-6d9bb337.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-0e3276c4.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-d243cf62.js'));
};
...
const guildHall = () => { runDefault(import('./hall-1519fada.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-bb832c4d.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-37982ed0.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-14acf789.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-2fafedb7.js')); };
const guildLog = () => { runDefault(import('./guildLog-13ae8541.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-befdf30a.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-adb847a4.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-5bb79fe9.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-ce62ef73.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-f074dd73.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-d5bb8602.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-a9076cb6.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-0e1b8ad0.js')); };
const outbox = () => { runDefault(import('./outbox-d1eebbc1.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-9c39651d.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-465ca170.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-48356c8d.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-adf5c46c.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-7bc4f0c7.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-6ebeef98.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-b9629313.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-84d80bfc.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-96adcb67.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-84921fca.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-261e6fbe.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-f2fb688a.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-2b982999.js')); };
const injectTopRated = () => { runDefault(import('./toprated-93806b2f.js')); };
...
const injectTrade = () => { runDefault(import('./trade-40038b88.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-90b8f887.js')); };
const injectBank = () => { runDefault(import('./injectBank-a18c385b.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-d6abe913.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-ebafd173.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-f08f1aa3.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-f85ff7b0.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-14ccbcdf.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-0ea36b74.js'));
};
const ladder = () => { runDefault(import('./ladder-c2e85e36.js')); };
const marketplace = () => { runDefault(import('./marketplace-edcb87f6.js')); };
const points = () => { runDefault(import('./points-a38ba5aa.js')); };
const superelite = () => { runDefault(import('./superelite-556acb1a.js')); };
...
export { $dataAccess as $, setText as A, setInnerHtml as B, getText as C, searchPlayerUrl as D, querySelector as E, querySelectorArray as F, playerLinkSelector as G, getTextTrim as H, getValue as I, defCharacterVirtualLevel as J, getElementsByClassName as K, defStatLevel as L, quickbuffUrl as M, arrayFrom as N, navigateTo as O, indexPhp as P, isArray as Q, once as R, clickThis as S, getUrlParameter as T, insertHtml as U, nowSecs as V, scouttowerUrl as W, setValue as X, sendEvent as Y, jQueryDialog as Z, injectNotepadShowLogs as _, add$1 as a, defStatArmor as a$, injectMonsterLog as a0, getCalfPrefs as a1, escapeHtml as a2, theLinks as a3, playerIdUrl as a4, notepadBlankUrl as a5, fallback as a6, now as a7, defLastLadderReset as a8, server as a9, retryAjax as aA, isFunction as aB, fromEntries as aC, containsText as aD, guideUrl as aE, defFetchPlayerBuffs as aF, defPlayerBuffs as aG, defPlayerUpdate as aH, defTeleport as aI, defPvE as aJ, isUndefined as aK, defAfterUpdateActionlist as aL, defFetchWorldRealmDynamic as aM, defFetchWorldRealmActions as aN, hideQTip as aO, worldUrl as aP, defRealmUpdate as aQ, defPlayerLevel as aR, keys as aS, defenderMultiplier as aT, defRelicView as aU, doSendGold as aV, defPlayerGold as aW, initSendGoldOnWorld as aX, sendGoldonWorld as aY, defStatAttack as aZ, defStatDefense as a_, cmdUrl as aa, pointsUrl as ab, insertHtmlAfterBegin as ac, querySelectorAll as ad, months as ae, sendException as af, parseError as ag, isObject as ah, defaults as ai, time as aj, timeEnd as ak, loadCss as al, defTr as am, infoBox as an, cdn as ao, arenaUrl as ap, oldActionSpinner as aq, defTd as ar, runDefault as as, composingUrl as at, defSubcmd as au, defNeedToCompose as av, contains as aw, defLastComposeCheck as ax, playerId as ay, showPlayerUrl as az, createDiv as b, newGuildLogUrl as b$, defStatDamage as b0, defStatHp as b1, defRefreshActionList as b2, defStairway as b3, defShopPrompt as b4, defViewCreature as b5, jsonParse as b6, defFetchPlayerBackpackCount as b7, defControlsKeydown as b8, handleEvent as b9, daViewProfile as bA, defEnableGuildActivityTracker as bB, draggable as bC, recallUserUrl as bD, defStatVl as bE, asyncPThree as bF, updateArchiveUrl as bG, archiveUrl as bH, auctionSearchUrl as bI, getValueJSON as bJ, setValueJSON as bK, profileUrl as bL, pCR as bM, pCL as bN, secureUrl as bO, tradeUrl as bP, classHandler as bQ, bountyUrl as bR, parseTemplePage as bS, displayDisconnectedFromGodsMessage as bT, blacksmithUrl as bU, dropItemsUrl as bV, defStatbarLevel as bW, injectFsBoxContent as bX, joinallUrl as bY, joinUnderUrl as bZ, newGuildLogLoc as b_, places as ba, mercRE as bb, ahSearchUrl as bc, getCustomUrlParameter as bd, attackplayerUrl as be, doAddIgnore as bf, combatSelector as bg, chatSelector as bh, noteSelector as bi, fshBuffLog as bj, screenview as bk, news$1 as bl, injectQuestBookFull as bm, inventing as bn, rarity as bo, itemType as bp, profile$2 as bq, composingFragmentType as br, isString as bs, guildLogUrl as bt, GMSTORAGE_PATH as bu, guildViewUrl as bv, lastActivityRE as bw, playerIDRE as bx, defCmd as by, guildRE as bz, calf as c, defJoinallgroupsundersize as c0, classPair as c1, injectGuild as c2, injectAuctionSearch as c3, injectOnlinePlayers as c4, injectFindOther as c5, injectFindBuffs as c6, injectRecipeManager as c7, injectBuffLog as c8, injectQuickLinkManager as c9, joinGroups as ca, auctionhouseUrl as cb, insertQuickExtract as cc, insertQuickWear as cd, dispatch as ce, defTable as d, entries as e, insertHtmlBeforeEnd as f, getElementsByTagName as g, hasClass as h, insertElement as i, jQueryPresent as j, on as k, itemRE as l, getArrayByTagName as m, cElement as n, onclick as o, pCC as p, extend as q, indexAjax as r, partial as s, createDocument as t, indexAjaxData as u, guildSubcmdUrl as v, infoBoxFrom as w, callApp as x, jQueryNotPresent as y, getElementById as z };
//# sourceMappingURL=calfSystem-c08399e5.js.map
