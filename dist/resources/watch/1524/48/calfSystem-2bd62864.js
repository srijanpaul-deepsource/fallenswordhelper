...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-2b758c82.js')); };
const injectGuild = () => { runDefault(import('./guild-37e6ff47.js')); };
const injectProfile = () => { runDefault(import('./profile-35a8fe4a.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-4a163a3b.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-937bf705.js')); };
const inventing = () => { runDefault(import('./inventing-8dbcdacd.js')); };
const news$1 = () => { runDefault(import('./news-6f4041af.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-e7f17e74.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-ffa87b58.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-3fb1332f.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-39bd436d.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-d3fce908.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-cf1f79c6.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-31bd1397.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-73fa7bfc.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-9ae0ae79.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-70f3a385.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-ba11cb28.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-ba11cb28.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-095f9cb7.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-095f9cb7.js').then((m) => m.injectFindOther(i)); };
...
function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-8a5814b1.js'));
  }
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-f81796e6.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-15489a7a.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-b5826054.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-9562591a.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-35bff209.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-737bf247.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-bc0d2b57.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-cd820fb8.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-b671f74e.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-f3375b70.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-3a75b8df.js'));
  }
}
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-335503ad.js')
...
function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-63421811.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-4db19faf.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-0a29c80a.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-78a9643b.js').then((m) => m.seLog());
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-a6bf55bb.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-804c63f8.js'));
  }
}

function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-4de0d387.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-637bd54c.js').then((m) => m.default(title));
}
...
const injectArena = () => { runDefault(import('./arena-9b1f6fd4.js').then(function (n) { return n.b; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-9a70d3ab.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-26ea4771.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-0f80f5f6.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-1664b3cc.js'));
};
const storeMoves = () => { runDefault(import('./store-95dbeb40.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-9dad44e6.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-41c09034.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-4e657505.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-18725fac.js')); };
const injectComposing = () => { runDefault(import('./composing-a798b896.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-353e8b06.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-383fd17f.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-b0df730c.js'));
};
...
const guildHall = () => { runDefault(import('./hall-d3335730.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-f3bc26a3.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-35fa31da.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-db66efc1.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-1ac6e220.js')); };
const guildLog = () => { runDefault(import('./guildLog-7818173d.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-814e5546.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-9945bde6.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-c5d7bd55.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-1d4c331d.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-ad982e1a.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-33c0b38e.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-141ac003.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-a0a28b03.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-043ba376.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-da5d38ea.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-c3d0f95b.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-f2b6616d.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-e794415c.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-cf93eaaf.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-ba98954b.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-8514b18c.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-bbecd3f1.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-01128562.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-11b59ac2.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-749d791a.js')); };
const injectTopRated = () => { runDefault(import('./toprated-65a4b252.js')); };
...
const injectTrade = () => { runDefault(import('./trade-25f5a8ec.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-0dd37c2c.js')); };
const injectBank = () => { runDefault(import('./injectBank-a702a93b.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-61c20939.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-e7b84ea9.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-41896c3a.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-daf183c1.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-b7c747b6.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-6bc70dd8.js'));
};
const ladder = () => { runDefault(import('./ladder-9db2a4df.js')); };
const marketplace = () => { runDefault(import('./marketplace-266181a7.js')); };
const points = () => { runDefault(import('./points-741da770.js')); };
const superelite = () => { runDefault(import('./superelite-14e1e428.js')); };
const privateMsg = () => { runDefault(import('./privateMsg-320c412f.js')); };
const playerLog = () => { runDefault(import('./playerLog-47833667.js')); };
const injectBuffmarket = () => { runDefault(import('./injectBuffmarket-c29b1665.js')); };
...
//# sourceMappingURL=calfSystem-2bd62864.js.map
