...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-1d04cdba.js')); };
const injectGuild = () => { runDefault(import('./guild-ef3e3353.js')); };
const injectProfile = () => { runDefault(import('./profile-cb28acde.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-41e679cd.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-66edfe7d.js')); };
const inventing = () => { runDefault(import('./inventing-cde69c09.js')); };
const news$1 = () => { runDefault(import('./news-f63a71ce.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-451603f0.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-5e8e26df.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-9ec9b8fe.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-46697659.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-508e3e13.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-c890db06.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-a8d8bfc0.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-d2aee510.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-8d3ea3fb.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-875fe071.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-5520cae0.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-5520cae0.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-ebffe85f.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-ebffe85f.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-15809171.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-b6e63b30.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-fdb12703.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-601fe7b5.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-16e777d9.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-4751fa75.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-e00210d9.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-59653808.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-0bf4c2ab.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-de0071b0.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-a1f72f8c.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-dc5170d0.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-741cb51c.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-f3dd1ef6.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-d39f689b.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-c32965e6.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-29667db2.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-691ddb46.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-34c91b83.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-b98f04c1.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-e3e9efa0.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-a3185f00.js').then(function (n) { return n.b; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-66fd1d4e.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-8f64f178.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-5c7b6a71.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-b82e3b2a.js'));
};
const storeMoves = () => { runDefault(import('./store-ba78d983.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-96f46463.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-d787a67f.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-2229184a.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-145ba4ae.js')); };
const injectComposing = () => { runDefault(import('./composing-04399884.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-a1112b1d.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-8494aeb3.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-2ecb8ee9.js'));
};
...
const guildHall = () => { runDefault(import('./hall-7a9456a4.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-657e2612.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-36aec009.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-b95ca887.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-bd93be8c.js')); };
const guildLog = () => { runDefault(import('./guildLog-69110456.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-02ae180b.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-e9db0680.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-139bb51c.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-4137085f.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-28192517.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-bc6652af.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-dbab6687.js')); };
...
const playerLog = () => { runDefault(import('./playerLog-86bddd50.js')); };
const outbox = () => { runDefault(import('./outbox-a82bcbab.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-4ba0e314.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-4f49ee22.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-9dc5bc8d.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-f5d88e3d.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-3431701e.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-29a2ff38.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-dccc8d1f.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-94d804ee.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-b6e821cf.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-2f178ff2.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-f9231ab6.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-19e9bc14.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-a0bcf813.js')); };
const injectTopRated = () => { runDefault(import('./toprated-6761c996.js')); };
...
const injectTrade = () => { runDefault(import('./trade-54508649.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-36935b20.js')); };
const injectBank = () => { runDefault(import('./injectBank-2f233041.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-beb8de65.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-8eed01fa.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-0c8181e8.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-d7893d59.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-90b7068d.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-f874d95e.js'));
};
const ladder = () => { runDefault(import('./ladder-af8e29fc.js')); };
const marketplace = () => { runDefault(import('./marketplace-9a6f7048.js')); };
const points = () => { runDefault(import('./points-e034cd5c.js')); };
const superelite = () => { runDefault(import('./superelite-a0e5f906.js')); };
...
//# sourceMappingURL=calfSystem-6840a38e.js.map
