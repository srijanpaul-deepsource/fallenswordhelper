...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-52dcdc82.js')); };
const injectGuild = () => { runDefault(import('./guild-18ae45fb.js')); };
const injectProfile = () => { runDefault(import('./profile-ade7f3af.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-63fbab66.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-5cb1fe8f.js')); };
const inventing = () => { runDefault(import('./inventing-197e3aae.js')); };
const news$1 = () => { runDefault(import('./news-fd10e5d1.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-77837205.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-90b4c712.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-12cb1ddd.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-8bf91e8a.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-4ad231d4.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-1f870961.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-de96ee5f.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-e49f2350.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-1a9b0f08.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-b5db5116.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-c4b0a6f8.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-c4b0a6f8.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-1e4db502.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-1e4db502.js').then((m) => m.injectFindOther(i)); };
...
function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-e3fd06ee.js'));
  }
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-f19a2894.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-b3ef40de.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-799c0ae9.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-6980f0e0.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-f3393bcd.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-9f4cc770.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-e5c29474.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-25b0bc99.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-99a235d0.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-ca0004d8.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-daf94b1b.js'));
  }
}
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-acc8efd6.js')
...
function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-7fc3812d.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-4e61faef.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-755daf25.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-9fc92cb6.js').then((m) => m.seLog());
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-076a59c6.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-ca7b6d5e.js'));
  }
}

function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-494e6fc8.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-603b7287.js').then((m) => m.default(title));
}
...
const injectArena = () => { runDefault(import('./arena-4ef7ca1c.js').then(function (n) { return n.b; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-09544cec.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-34cc68ce.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-808176fc.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-caf9c907.js'));
};
const storeMoves = () => { runDefault(import('./store-4fa11ec5.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-b6020489.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-1859ecb7.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-df6a1cc6.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-ddb575b6.js')); };
const injectComposing = () => { runDefault(import('./composing-e6a9d675.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-61a21b57.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-ba98f605.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-03237669.js'));
};
...
const guildHall = () => { runDefault(import('./hall-720da7ce.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-710f2bc4.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-9e967a9a.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-73714121.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-3a41b8bc.js')); };
const guildLog = () => { runDefault(import('./guildLog-7e88ab3b.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-4871fcce.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-745dc45c.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-8057f4d4.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-a6c254a6.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-9cec017f.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-159a6e85.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-c596d0f3.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-021e5de4.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-735959cd.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-10f37993.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-cad0b361.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-5937844e.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-47629df1.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-dfa306bf.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-aa7287cf.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-874b712c.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-8bf73a93.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-31c9a599.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-f2558949.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-cbbae7fe.js')); };
const injectTopRated = () => { runDefault(import('./toprated-f9abcefb.js')); };
...
const injectTrade = () => { runDefault(import('./trade-f92e2987.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-c8f80769.js')); };
const injectBank = () => { runDefault(import('./injectBank-73d42fa9.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-ce96908c.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-70202b68.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-9abdc533.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-d53e62e8.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-7553a74d.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-e5cc84bd.js'));
};
const ladder = () => { runDefault(import('./ladder-e5cd3b31.js')); };
const marketplace = () => { runDefault(import('./marketplace-87df952c.js')); };
const points = () => { runDefault(import('./points-fa654f07.js')); };
const superelite = () => { runDefault(import('./superelite-7bacb06b.js')); };
const privateMsg = () => { runDefault(import('./privateMsg-4d818a81.js')); };
const playerLog = () => { runDefault(import('./playerLog-83b76c61.js')); };
const injectBuffmarket = () => { runDefault(import('./injectBuffmarket-58d9f721.js')); };
...
//# sourceMappingURL=calfSystem-579856fa.js.map
