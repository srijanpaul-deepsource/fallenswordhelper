...
const injectBioWidgets = () => { runDefault(import('./bioWidgets-09bd2049.js')); };
const injectGuild = () => { runDefault(import('./guild-962521dd.js')); };
const injectProfile = () => { runDefault(import('./profile-7e0d9462.js').then(function (n) { return n.p; })); };
const injectProfileDropItems = () => { runDefault(import('./injectProfileDropItems-d2f68446.js')); };
const injectQuestBookFull = () => { runDefault(import('./injectQuestBookFull-eef9b5f2.js')); };
const inventing = () => { runDefault(import('./inventing-de71a61e.js')); };
const news$1 = () => { runDefault(import('./news-b89f9252.js')); };
const ufsgAllowBack = () => { runDefault(import('./ufsgAllowBack-6e79550f.js')); };
const viewArchive = () => { runDefault(import('./viewArchive-ef3c6760.js')); };

const injectBuffLog = (i) => { import('./injectBuffLog-0d48304d.js').then((m) => m.default(i)); };
const injectFsBoxContent = (i) => { import('./injectFsBoxContent-9ad7ae23.js').then((m) => m.default(i)); };
const injectMonsterLog = (i) => { import('./monstorLog-2126bf5e.js').then((m) => m.default(i)); };
const injectNotepadShowLogs = (i) => { import('./combatLog-57eb0ffe.js').then((m) => m.default(i)); };
const injectOnlinePlayers = (i) => { import('./injectOnlinePlayers-49648302.js').then((m) => m.default(i)); };
const injectRecipeManager = (i) => { import('./recipeMgr-7cf7e1df.js').then((m) => m.default(i)); };
const insertQuickExtract = (i) => { import('./quickExtract-d6a91b16.js').then((m) => m.default(i)); };
const insertQuickWear = (i) => { import('./quickWear-a0d27667.js').then((m) => m.default(i)); };

const injectAuctionSearch = (i) => { import('./lists-867be5bd.js').then((m) => m.injectAuctionSearch(i)); };
const injectQuickLinkManager = (i) => { import('./lists-867be5bd.js').then((m) => m.injectQuickLinkManager(i)); };
const injectFindBuffs = (i) => { import('./findBuffs-de826c1e.js').then((m) => m.injectFindBuffs(i)); };
const injectFindOther = (i) => { import('./findBuffs-de826c1e.js').then((m) => m.injectFindOther(i)); };
...
function getLinkConfig(theNav, myNav) {
  ...
  if (linkConfig.some(([, b]) => b)) {
    import('./injectItems-2caef0e4.js')
...
function moveUp(title) {
  import('./moveRHSBoxUpOnRHS-650c980b.js').then((m) => m.default(title));
}

function moveLeft(title) {
  import('./moveRHSBoxToLHS-4f04b573.js').then((m) => m.default(title));
}
...
function callAllyEnemy() {
  if (calf.enableAllyOnlineList
    ...
    runDefault(import('./allyEnemy-6356e62e.js'));
  }
}
...
function callBounties() {
  if (calf.enableWantedList
    ...
    runDefault(import('./activeWantedBounties-58521296.js'));
  }
}

function callGuildInfo() {
  if (calf.enableGuildInfoWidgets) {
    runDefault(import('./addGuildInfoWidgets-67062cca.js'));
  }
}

function callAllies() {
  if (calf.enableOnlineAlliesWidgets) {
    runDefault(import('./addOnlineAlliesWidgets-b04bb75a.js'));
  }
}

function callTemple() {
  if (calf.enableTempleAlert) {
    runDefault(import('./injectTempleAlert-05678df8.js'));
  }
}

function callUpgrade() {
  if (calf.enableUpgradeAlert) {
    runDefault(import('./injectUpgradeAlert-fc2e8372.js'));
  }
}

function callComposing() {
  if (calf.enableComposingAlert) {
    runDefault(import('./injectComposeAlert-b936fc8d.js'));
  }
}

function statbar() {
  if (getValue('statBarLinks')) {
    runDefault(import('./statBar-8a7eb4ac.js'));
  }
}

function staminaCalc() {
  if (getValue('staminaCalculator')) {
    runDefault(import('./injectStaminaCalculator-e53e507c.js'));
  }
}

function levelCalc() {
  if (getValue('levelUpCalculator')) {
    runDefault(import('./injectLevelupCalculator-f45eb581.js'));
  }
}

function fsBoxLog() {
  if (getValue('fsboxlog')) {
    runDefault(import('./injectFSBoxLog-79afbad9.js'));
  }
}

function expandQb() {
  if (getValue('resizeQuickBuff')) {
    runDefault(import('./interceptQuickBuff-e6e11364.js'));
  }
}

function joinAll() {
  if (getValue('joinAllLink')) {
    runDefault(import('./injectJoinAllLink-b0116d75.js'));
  }
}

function guildLogHref() {
  if (getValue('useNewGuildLog')) {
    runDefault(import('./changeGuildLogHREF-0ea4184e.js'));
  }
}

function gameStats() {
  if (getValue('addServerNode')) {
    runDefault(import('./injectServerNode-b686a7f6.js'));
  }
}

function scoutTower() {
  if (getValue('addScoutTowerLink')) {
    runDefault(import('./scoutTowerLink-949e0e0a.js'));
  }
}

function guildActivityTracker() {
  if (jQueryPresent() && getValue(defEnableGuildActivityTracker)) {
    runDefault(import('./guildActivity-28d2f99f.js'));
  }
}

function seTracker() {
  if (jQueryPresent() && calf.enableSeTracker && calf.cmd !== 'superelite') {
    import('./seLog-3307b19c.js').then((m) => m.seLog());
  }
}
...
const injectArena = () => { runDefault(import('./arena-5cb60233.js').then(function (n) { return n.b; })); };
const arenaDoJoin = () => { runDefault(import('./arenaDoJoin-560affec.js')); };
const arenaJoin = () => {
  runDefault(import('./arenaJoin-477762c4.js'));
};
const completedArenas = () => {
  runDefault(import('./completedArenas-f1d0e05f.js'));
};
const setupMoves = () => {
  runDefault(import('./setup-03b10ddd.js'));
};
const storeMoves = () => { runDefault(import('./store-4d388283.js')); };
...
const injectAuctionHouse = () => {
  runDefault(import('./injectAuctionHouse-d9ae0a9f.js'));
};
const quickCreate = () => {
  runDefault(import('./quickCreate-a7891226.js'));
};
...
const composingBreakdown = () => { runDefault(import('./breakdown-7045a912.js')); };
const composingCreate = () => { runDefault(import('./composingCreate-bdb36464.js')); };
const injectComposing = () => { runDefault(import('./composing-ffc194ef.js')); };
...
const injectAdvisor = () => {
  runDefault(import('./guildAdvisor-002e0fcf.js'));
};
...
const injectGroups = () => {
  runDefault(import('./groups-cfb86c5e.js'));
};
const injectGroupStats = () => {
  runDefault(import('./injectGroupStats-d4de8b1d.js'));
};
...
const guildHall = () => { runDefault(import('./hall-61e1f21a.js')); };
...
const injectReportPaint = () => {
  runDefault(import('./guildReport-ac483eb6.js'));
};
const injectGuildAddTagsWidgets = () => {
  runDefault(import('./injectGuildAddTagsWidgets-f9bda0d1.js'));
};
const storeitems = () => {
  runDefault(import('./storeitems-464a978a.js'));
};
...
const guildChat = () => { runDefault(import('./guildChat-4c44da95.js')); };
const guildLog = () => { runDefault(import('./guildLog-55872e59.js')); };
const guildMailbox = () => {
  runDefault(import('./guildMailbox-c3a5c5ef.js'));
};
const injectGuildBank = () => {
  runDefault(import('./injectGuildBank-247b11c6.js'));
};
const injectGuildRanks = () => {
  runDefault(import('./rank-3b80d2e3.js'));
};
const injectRPUpgrades = () => {
  runDefault(import('./injectRPUpgrades-1095dfb7.js'));
};
const injectScouttower = () => {
  runDefault(import('./injectScouttower-92d3c6d6.js'));
};
...
function injectWorld() {
  if (isNewMap()) {
    runDefault(import('./newMap-91e04bac.js'));
  }
}

const itemsView = () => { runDefault(import('./itemsView-a2c2fd1d.js')); };
...
const newsFsbox = () => { runDefault(import('./newsFsbox-d1d3d99b.js')); };
const newsShoutbox = () => { runDefault(import('./newsShoutbox-be0849f8.js')); };
...
const unknownPage = () => { runDefault(import('./unknownPage-5965c555.js')); };
...
const injectInventoryManagerNew = () => {
  runDefault(import('./inventory-1a7ab62d.js'));
};
const injectNewGuildLog = () => {
  runDefault(import('./newGuildLog-40d1d00c.js'));
};
const injectNotepad = () => {
  runDefault(import('./injectNotepad-cfd2e0c1.js'));
};
const injectSaveSettings = () => { runDefault(import('./load-57444d79.js')); };
const reliclist = () => {
  runDefault(import('./reliclist-5e9b7d26.js'));
};
const whosGotWhat = () => {
  runDefault(import('./whosGotWhat-06453818.js'));
};
...
const injectQuestTracker = () => {
  runDefault(import('./injectQuestTracker-9b419b15.js'));
};
...
const showAllQuestSteps = () => {
  runDefault(import('./showAllQuestSteps-e1eaf900.js'));
};
...
const injectScavenging = () => {
  runDefault(import('./scavenging-8767ff40.js'));
};
...
const globalQuest = () => { runDefault(import('./globalQuest-6714ab1d.js')); };
const injectTopRated = () => { runDefault(import('./toprated-54f7f5ed.js')); };
...
const injectTrade = () => { runDefault(import('./trade-d4dcc8d9.js')); };
...
const craftForge = () => { runDefault(import('./craftForge-1d1c20ad.js')); };
const injectBank = () => { runDefault(import('./injectBank-778709dc.js')); };
const injectBazaar = () => { runDefault(import('./bazaar-2b22a3a2.js')); };
const injectFindPlayer = () => {
  runDefault(import('./injectFindPlayer-9c7177e8.js'));
};
const injectMailbox = () => { runDefault(import('./mailbox-09d8cc7d.js')); };
const injectQuickBuff = () => {
  runDefault(import('./quickBuff-3f7c6135.js'));
};
const injectTitan = () => { runDefault(import('./injectTitan-ca8797b8.js')); };
const injectSettings = () => {
  runDefault(import('./injectSettings-9dd26a9c.js'));
};
const ladder = () => { runDefault(import('./ladder-01e625e5.js')); };
const marketplace = () => { runDefault(import('./marketplace-2341b0f3.js')); };
const points = () => { runDefault(import('./points-4620b89d.js')); };
const superelite = () => { runDefault(import('./superelite-9f41804b.js')); };
const privateMsg = () => { runDefault(import('./privateMsg-3117f23b.js')); };
const playerLog = () => { runDefault(import('./playerLog-29a1a2bf.js')); };
