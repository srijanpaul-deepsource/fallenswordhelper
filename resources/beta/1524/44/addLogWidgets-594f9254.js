import{e as n,ah as e,E as a,ai as t,aj as s,F as r,i as o,ak as i,H as c,x as l,u as f,t as u,v as d,z as m,C as p,S as h,$ as g,U as b,al as w,D as y,am as _,a9 as v,h as $,m as C,A as P,I as k,a5 as j,an as L,Z as T,y as G}from"./calfSystem-a8d582b6.js"
import{i as S}from"./insertHtmlAfterEnd-7c6126dc.js"
import{c as A}from"./currentGuildId-6d2e1d9d.js"
import{g as E}from"./getMembrList-ba181b15.js"
import{c as x}from"./closestTr-9db047d3.js"
import{a as I}from"./addCommas-aab57e5e.js"
import{p as R}from"./parseDateAsTimestamp-e85e174f.js"
import{g as z,s as B}from"./idb-d8c9a883.js"
import{c as Y}from"./createStyle-d4f6dc05.js"
import{m as H}from"./myStats-a3bdf6c3.js"
import{g as M}from"./addLogColoring-44e5420a.js"
let N
...
function F(n,[e,a]){o(a.cells[1],`<font size="1"><br>[ ${((n,e)=>`<a${(n=>n?' class="pmIgnoreUrl"':"")(n)} href="${i}${c(e)}" data-tooltip="Add to Ignore List">Ignore</a>`)(n,e)} ]</font>`)}function W(n){return l({cmd:"combat",subcmd:"view",combat_id:n})}function X(n){return Number(n.getAttribute("background").match(/\/(\d+)/)[1])}function q(n,e){const a=new RegExp(`${e} = (\\d+)`)
...
return e?{id:e[0],params:[e[1][1],e[1][2]]}:(h("Logs","Missing PvP Special",n),{id:-1,params:["-1","-1"]})}function O(n){return a("#specialsDiv",n).map(c).filter((n=>["leeched","Spell"].some((e=>n.includes(e))))).map(K)}function Q(n){return{id:X(n.rows[1].cells[0]),name:c(n.rows[0].cells[0])}}function V(n){return{id:X(n.rows[1].cells[2]),name:c(n.rows[0].cells[2])}}function nn(n,e){const a=e.children[0].rows[5].cells[0].children[0]
return{attacker:Q(a),defender:V(a),id:Number(n),specials:O(e)}}function en(n){const e=p(n.children[1])
return{gold_gain:q(e,"goldGain"),gold_stolen:q(e,"goldStolen"),pvp_prestige_gain:q(e,"prestigeGain"),pvp_rating_change:q(e,"pvpRatingChange"),winner:q(e,"winner"),xp_gain:q(e,"xpGain")}}function an(n,e){return{...nn(n,e),...en(e)}}function tn(n,e){const a=d(e)
return{r:an(n,m("pCC",a)),s:!0}}function sn(n){return function(n){return f({cmd:"combat",subcmd:"view",combat_id:n})}(n).then(u(tn,n))}let rn,on
...
var s
if(t&&t.s)return on||(on={...a}),on[e]={...t,logTime:R(c(n.cells[1]))/1e3},B("fsh_pvpCombat",on),t}async function un(n,e){rn||(rn=ln())
const a=await rn
return a[e]&&a[e].logTime?a[e]:fn(n,e,a)}const dn=n=>y(_,n),mn=([,n])=>!/\(Guild Conflict\)/.test(n),pn=async([n,e])=>[n,e,await un(n,/combat_id=(\d+)/.exec(e)[1])]
...
const a=c(n),[t,s,r]=await Promise.all([D(a),$n(a),Cn(a)])
...
if(0===e.length)return;(await Promise.all(function(n){return n.map(x).filter(dn).map((n=>[n,n.cells[2].innerHTML])).filter(mn).map(pn)}(e))).filter(wn).forEach(bn)}]]
function Rn(n,e,a){k(a[0])&&a[1](n,e)}function zn(n){if(G())return
const e=M()
e&&function(n,e){In.forEach(u(Rn,n,e))}(e,n)}export{zn as a}
//# sourceMappingURL=addLogWidgets-594f9254.js.map
